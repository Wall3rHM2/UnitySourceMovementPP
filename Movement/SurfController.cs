...
namespace Fragsurf.Movement {
    public class SurfController {
        ...
        public void ProcessMovement (ISurfControllable surfer, MovementConfig config, float deltaTime) {
            ...
            } else if (_surfer.moveData.noclip) {
                Noclip();
            } else {
        ...
        private void Noclip() {
            ...
            float speed = _config.noclipSpeed;
            if (Input.GetButton ("Sprint")) speed = _config.noclipSprint;
            if (Input.GetButton ("Jump"))
                _surfer.moveData.velocity.y = speed / 2;

            float camX = _surfer.moveData.camX;
            ...
            float aimVert = (-camX / 90f) * _surfer.moveData.verticalAxis;
            float forwardMove = _surfer.moveData.verticalAxis;
            float rightMove = _surfer.moveData.horizontalAxis;
            ...
            _wishDir = forwardMove * forward + rightMove * right;
            _wishDir.Normalize();
            _wishDir *=  1 - Mathf.Abs(aimVert);
            Vector3 moveDirNorm = _wishDir;

            // go up and down based on cam vertical axis
            _surfer.moveData.velocity.y += (aimVert * speed) / 2;

            float yVel = _surfer.moveData.velocity.y;
            //_wishDir = _wishDir * _wishDir.magnitude;
            _wishDir *= speed;
            _surfer.moveData.velocity.x = _wishDir.x;
            _surfer.moveData.velocity.z = _wishDir.z;
